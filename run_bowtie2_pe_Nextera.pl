...
foreach $file (@files)
{
	...
	#get sam file
	$cmd_sam="/volumes/neo/code/3rd_party/bowtie2-2.2.6/bowtie2 -x /volumes/neo/code/3rd_party/bowtie2-2.2.6/hg19/hg19 -1 $file -2 $seq2 -S $samName --local --rg-id $sampleName --rg SM:$sampleName --rg PU:$sampleName --rg PL:ILLUMINA --rg LB:$sampleName -p 6 ";
...
$cmd="/volumes/neo/code/3rd_party/samtools-1.2/samtools view -uF 0x400 -q 1 $key.marked.sorted.bam | /volumes/neo/code/3rd_party/BEDTools-Version-2.14.3/bin/coverageBed -abam - -b /volumes/neo/database/Nextera/NexteraRapidCapture_Exome_TargetedRegions_hg19_clean.bed -d >& $outfile";
system("$cmd");
