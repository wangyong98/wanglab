copyNumberProcessing <- function(dataName = dataName, dataDir = dataDir, outputDir = outputDir, gcName = "/Volumes/sci/code/yongwang_hg19/varbin.gc.content.50k.bowtie.k50.hg19.txt", alpha = 0.01){
...
if(!file.exists(paste(outputDir,"MATLAB_matrix", sep="/")))
{dir.create(paste(outputDir,"MATLAB_matrix", sep="/"))}
...
for(x in 12:ncol(cnRatio))
{
  ...
  segment.smoothed.CNA.object <- segment(smoothed.CNA.object, alpha=alpha, min.width=5, undo.splits="sdundo", undo.SD=2) 
